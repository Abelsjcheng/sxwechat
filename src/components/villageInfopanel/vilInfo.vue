...
<!-- 文化活动组件界面 -->
  <div>
      <div v-transfer-dom>
        <popup v-model="showvilInfo" position="right" width="100%">
          <x-header :left-options="{preventGoBack: true}" @on-click-back="backpage">乡村资讯信息</x-header>
         <div>
      ...
      <div v-transfer-dom>
        <popup v-model="show" position="right" width="100%">
          <div>
            <x-header class="vux-scroller-header" :left-options="{preventGoBack: true}" @on-click-back="backpagevilInfo"></x-header>
            <div>
  ...
  methods: { // 方法函数
    ...
    backpagevilInfo () { // 关闭详情弹窗
     ...
     getImginfo:function () { // 数据请求函数
      this.axios.get('http://110.53.162.165:5050/api/beaCountry/ListBeaCountry?',{params:{vtype:6,pageIndex:1,pageSize:20 } }).then((res) => {
