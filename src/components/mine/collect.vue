        ...
        <view-box>
           <div slot="header">
            ...
            <scroller lock-x scrollbar-y use-pullup use-pulldown @on-pullup-loading="loadMore" @on-pulldown-loading="refresh" @on-scroll="onscroll" ref="scrller" height="-46" v-model="status">
                <div>
                     <panel :list="list" type="5" style="margin-top:0px;" @on-click-item="openproject"></panel> 
                </div>
                <!--上拉刷新-->
...
import { ViewBox, XHeader, Panel, Scroller, LoadMore, Popup, TransferDom  } from 'vux'
export default {
  name: 'collect', // 我的收藏
  directives: {
    TransferDom
  },
  components: { ViewBox, XHeader, Panel, Scroller, LoadMore, Popup, infocontent }, // 注册组件
  data() {
      return {
        show:false,
        status: {
